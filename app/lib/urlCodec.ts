/**
 * URL codec for encoding/decoding game state
 */

import type { GameState, Player, GameResults, TakeoutMeal } from './gameState';

/**
 * Encode game state to URL parameters
 */
export function encodeGameState(state: GameState): URLSearchParams {
 ...
 */
export function decodeGameState(params: URLSearchParams | string | null): GameState | null {
  try {
    // Handle string input
    ...
    // Settings
    const settings = {
      mealCount: parseInt(params.get('mc') || '5') as 3 | 5 | 7 | 10,
      budgetCap: (params.get('bc') || 'moderate') as 'tight' | 'moderate' | 'fancy' | 'none',
    ...
    const takeoutMeals: TakeoutMeal[] = [];
    if (takeoutStr) {
    ...
    const theme = params.get('th') || undefined;
    const playerBName = params.get('pBNm') || undefined;
    const checkedIngredients = params.get('chk') ? params.get('chk')!.split('|') : [];
    ...
    return {
      ...
      pool: pool as any[],
      results,
      status: (params.get('status') || 'setup') as GameState['status'],
      currentRound,
      harmoniesSoFar,
      usedMeals,
      playerAAllPicks,
